 ...
 *
 * Ported from src/pages/index.js. The page itself is now a server component
 * that fetches and passes `projects` down, so the GROQ, the Sanity token and
 * the fetch never reach the browser — only this leaf does.
 *
 * Gatsby's `navigate()` becomes next/navigation's router.push().
 */
export default function HomeProjects({ projects }) {
  ...
  const handlePasswordCorrect = () => {
    if (selectedProject) {
      router.push(`/case-study/${selectedProject.slug.current}`)
    }
  }
