...
// Served from public/, not bundled — Next resolves these at runtime.
const LeftActive = "/svg/Left_Active.svg"
const LeftDisabled = "/svg/Left_Disabled.svg"
const RightActive = "/svg/Right_Active.svg"
const RightDisabled = "/svg/Right_Disabled.svg"
const InfoIcon = "/svg/Info.svg"
...
const VideoComponent = ({ videoFile, videoUrl, posterImage, autoplay, loop, muted, caption }) => {
  try {
    ...
    // Build-time and runtime-unlocked content now carry the same plain asset
    // URL, so there is a single path here — gatsbyImageData no longer exists.
    const posterSrc = sanityImageUrl(posterImage?.asset?.url, { width: 1280 }) || null
...
export default function CaseStudyContent({
  ...
  // The server component decides what to fetch: for a locked project it never
  // requests the body at all, so `staticCaseStudy` arrives null and the shell
  // carries title/client/year/hero. That replaces the __locked__ sentinel
  // gatsby-node.js needed, because a server component can simply branch.
